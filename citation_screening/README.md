...
- 仅纳入“存疑”和“领域不符”，不向作者展示匹配项、未获取数据或引用无关内容。
- 每条以参考文献编号开头，并包含合并后的前文、目标句和后文、英文问题说明及建议操作。
- 原稿句子由程序从结果 JSON 原样写入，DeepSeek 只润色英文问题与建议，避免改写或虚构作者原文。
- 每 15 条合并为一次 DeepSeek 请求，生成前显示预计调用量，并按实际请求次数结算额度。
- 英文 HTML 报告会与其他任务一样保存 24 小时，可从“最近任务”重新下载。
